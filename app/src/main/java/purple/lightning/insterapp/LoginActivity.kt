...
import purple.lightning.insterapp.databinding.ActivityLoginBinding
import java.io.File
...
class LoginActivity : AppCompatActivity(), LocalUserItemAdapter.OnUserClickListener {
    ...
    private  var database: DatabaseReference = FirebaseDatabase.getInstance("https://insterapp-ac6ef-default-rtdb.firebaseio.com/").getReference("Users")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val loggedInUsers: List<LoggedInUsersEntity> = loggedInUsersDao.fetchAllLoggedInUsers()
    ...
    private fun setUpLocalUsersRv(loggedInUsers: List<LoggedInUsersEntity>){
    ...
    override fun onUserClickListener(position: Int, username: String) {
        ...
        val intent = Intent(this@LoginActivity, InsterActivity::class.java)
        startActivity(intent)
        finish()
    }
        ...
        database.child(username).get().addOnSuccessListener {
            if(it.exists()){
                ...
                if (decryptedPassword == password.uppercase()){
                    ...
                    val userRoom = LoggedInUsersEntity(username=username, password = dbPassword.uppercase(), isLoggedIn = true)
                    val loggedInUsersDao = (application as RoomApp).db.loggedInUsersDao()
    ...
    private fun addtoLoggedInUsers(userRoom: LoggedInUsersEntity, loggedInUsersDao: LoggedInUsersDao){
        lifecycleScope.launch {
